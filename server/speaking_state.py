...
class SpeakingStateObserver(BaseObserver):
    ...
    async def wait_until_bot_stops(self) -> None:
        """Return immediately if the bot is idle; otherwise suspend until the next
        BotStoppedSpeakingFrame. Exactly one queued waiter is released per transition,
        so concurrent callers serialize naturally across successive speaking cycles."""
